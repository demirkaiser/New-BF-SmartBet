...
# General --
# 1 You don't get redirected to IceBear's affiliate link anymore;
# 2 Option to disable updating to newest chrome drivers;
# 3 Correctly logs in on new site;

# New site --
# - UPDATED CSS SELECTORS;
# 1 Is able to correctly set bet amount upon starting;
# 2 Correctly updates bet- and multiplier amount;
# 3 Joins games after updating everything;
...
checkDriver = False # enable for checking and updating chrome driver to newest version -- keep on `False` for faster loading time
...
class main:
	...
	def getConfig(self): # Get configuration from config.json file
		...
		with open("config.json", "r+") as data:
			...
			# GREATLY IMROVED LOADING TIMES
			browser = self.browser
			browser.get("https://bloxflip.com/") # Open browser
			while True:
				try:
	...
	def updateBetAmount(self, amount):
		print(f"trying to update amount: {amount}")
		browser = self.browser
	...
	def updateMultiplier(self, multiplier):
		print(f"trying to update multiplier: {multiplier}")
		browser = self.browser
